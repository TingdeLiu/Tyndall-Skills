...
- **核心功能：**
  ...
  - **彩蛋宠物 🐣：** 状态栏行尾住着一只金色卡皮巴拉 —— 它会摆姿势（`\(^ww^)/` `~(-oo-)~`）、嚼东西、眨眼、单眼眨，并根据上下文用量 / 花费 / 速率限制在 **9 种心情**间轮换，每种心情都有自己的符号和短语池（`♥♥♥ chef's kiss` · `zZz /compact soon?` · `!?! everything is soup`）。它还会**随着你干活解锁颜色** —— 默认全金，当天 API 工作时长越过 1 / 3 / 8 小时后依次解锁彩色符号、彩色脸、整只流动彩虹。纯本地运行，**0 token 消耗**。
- **设置与前提条件：**
...
|---|:--:|---|
| < 1 h | 1 | 全金色 —— `(^oo^) ♥♥♥` |
| 1 – 3 h | 2 | **符号**按心情上色（开心粉、发呆蓝、零食橙…） |
| 3 – 8 h | 3 | **脸和符号**各自上色 —— 脸用淡色，符号用亮色 |
...
| 环境变量 | 作用 |
|---|---|
| `BUDDY_TIERS="60,180,480"` | 自定义解锁档 2/3/4 的分钟阈值 |
